...
def get_ptb(folder_path, seq_length=64):
    ...
    data = [[word2idx[word] for word in text[:-(len(text) % seq_length)]]
            for text in texts]
    train_data, eval_data, test_data = [
        np.array(d).reshape(-1, seq_length) for d in data
    ]
...
def get_estimator(init_lr,
                  ...
                  epochs=98,
                  control_frequency=3):
    train_data, _, test_data = get_ptb(folder_path=data_dir,
                                       seq_length=seq_length + 1)
    pipeline = fe.Pipeline(
        train_data=fe.dataset.NumpyDataset(data={"x": train_data}),
        eval_data=fe.dataset.NumpyDataset(data={"x": test_data}),
        batch_size=batch_size,
        ops=CreateInputAndTarget(inputs="x", outputs=("x", "y")),
        drop_last=True)
    # step 2
    model = fe.build(model_fn=lambda: build_model(
        vocab_size, embedding_dim=300, rnn_units=600, batch_size=batch_size),
                     optimizer_fn=lambda: tf.optimizers.SGD(
                         init_lr, momentum=0.9))  #1.0, 0.1, 0.01
