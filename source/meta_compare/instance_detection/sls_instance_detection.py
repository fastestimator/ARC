...
class RetinaLoss(TensorOp):
    def forward(self, data, state):
        (cls_pred, loc_pred), anchorbox = data
...
class SGDLinesSearch(fe.op.tensorop.TensorOp):
    ...
    def forward(self, data, state):
        image, anchorbox = data
        closure = lambda: self.loss_op.forward((self.model(image), anchorbox), state=state)
        self.opt.zero_grad()
...
def get_estimator(data_dir, batch_size=32, epochs=30, max_train_steps_per_epoch=1500, image_size=256, num_classes=90):
    # pipeline
    ...
    pipeline = fe.Pipeline(
        ...
        pad_value=0)
    # network
    model = fe.build(model_fn=lambda: RetinaNet(num_classes=num_classes), optimizer_fn="sgd")
    opt = sls.Sls(model.parameters())
    network = fe.Network(ops=[
        ModelOp(model=model, inputs="image", outputs="all_pred"),
        SGDLinesSearch(model=model,
                       opt=opt,
                       loss_op=RetinaLoss(inputs=["all_pred", "anchorbox"], outputs="total_loss"),
                       inputs=("image", "anchorbox"),
                       outputs="total_loss"),
        RetinaLoss(inputs=["all_pred", "anchorbox"], outputs="total_loss", mode="eval"),
        DummpyUpdate(model=model, loss_name="total_loss"),
        PredictBox(input_shape=(image_size, image_size, 3), inputs="all_pred", outputs="pred", mode="eval")
    ])
    # estimator
    traces = [
        MeanAveragePrecision(num_classes=num_classes, true_key='bbox', pred_key='pred', mode="eval"), PrintLR(opt=opt)
    ]
